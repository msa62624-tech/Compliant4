...
interface GCLoginProps {
  onLogin?: (gcData: GCData) => void;
}

export default function GCLogin({ onLogin }: GCLoginProps): JSX.Element {
  ...
  const [multipleGCs, setMultipleGCs] = useState<GCData[] | null>(null);

  const submit = async (e: React.FormEvent<HTMLFormElement>, gcIdOverride: string | null = null): Promise<void> => {
    e.preventDefault();
    setLoading(true);
    setError(null);
    try {
      ...
      const gcData: GCData = await response.json();
          ...
          <Card className="shadow-2xl border-0">
            ...
            <CardContent className="p-8">
              <div className="space-y-3">
                {multipleGCs.map((gc: GCData) => (
                  <button
                    key={gc.id}
                    onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                      submit(e as any, gc.id);
                    }}
                    disabled={loading}
