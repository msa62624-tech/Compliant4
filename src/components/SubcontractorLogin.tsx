...
interface ContractorLoginResponse {
  ...
  contractors?: SubcontractorData[];
  contractor?: SubcontractorData;
}

interface SubcontractorLoginProps {
  onLogin?: (subId: string) => void;
}

export default function SubcontractorLogin({ onLogin }: SubcontractorLoginProps): JSX.Element {
  ...
  const [multipleSubs, setMultipleSubs] = useState<SubcontractorData[] | null>(null);

  const submit = async (e: React.FormEvent<HTMLFormElement>, subIdOverride: string | null = null): Promise<void> => {
    e.preventDefault();
    setLoading(true);
    setError(null);
    try {
      ...
      const data: ContractorLoginResponse = await response.json();
          ...
          <Card className="shadow-2xl border-0">
            ...
            <CardContent className="p-8">
              <div className="space-y-3">
                {multipleSubs.map((sub: SubcontractorData) => (
                  <button
                    key={sub.id}
                    onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                      submit(e as any, sub.id);
                    }}
                    disabled={loading}
