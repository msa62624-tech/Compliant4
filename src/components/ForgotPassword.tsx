 ...
 */
export default function ForgotPassword({ onBackToLogin, portalType = 'gc' }: ForgotPasswordProps): JSX.Element {
  ...
  const requestReset = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    setLoading(true);
    setError(null);
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to request reset';
